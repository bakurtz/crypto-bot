...
module.exports = () => new Promise((resolve,reject)=>{
            ...
            openDbOrders.forEach(dbOrder => {
                ...
                if(openCBOrders.length>0){
                    for(let i=0;i<openCBOrders.length;i++){
                        if(dbOrder.id===openCBOrders[i].id){
                            ...
                            if(dbOrder.filled_size!=openCBOrders[i].extra.filled_size){
                                ...
                                authClient.getFills(fillFilter, (fills)=>{
                                    if(fills.length===0){
                    ...
                    if(!foundMatch){
                        ...
                        coinbasePro.loadOrder(dbOrder.id).then((order: LiveOrder) => {
                            let o = convertOrderType(order);
                            //Grab Fills for this order
                            instance.post('/updateOrder',{params: {order: o}})
                            .then((resp) => {
    ...
    function getAllOpenDBOrders(){
        instance.get('/getOpenOrders')
        .then((resp) => {
...
// const updateDbOrder = (order: Order) => {
//     instance.post('/updateOrder',{params: {order}})
//     .then((resp) => {
//         console.log("updateDbOrder API CALL SUCCEEDED");
//         openDbOrders=resp.data.data;
//         callsComplete++;
...
// const queryCBFills = (orderId: string) => {
//     let fillFilter: FillFilter = {
//         product_id: product,
//         order_id: orderId
//     }
...
// const queryDBFills = (orderId: string) => {
//     instance.get('/getOrder',{params: {orderId}})
//     .then((resp) => {
//         console.log("queryDBFills API CALL SUCCEEDED")
//     })
//     .catch(err => console.log("queryDBFills API CALL FAILED"));
// }

/*
    RECONCILE
    1. Request order data from FILLS API on each item in needsUpdate list
    2. Update orders collection in DB with new data (fees, fill date, etc)
        Be careful because this may be a one-to-many relactionship (multiple fills per order)
*/